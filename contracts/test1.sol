...
contract PatientRecords is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    // Events
    event RecordModified(uint256 indexed patientId, uint256 timestamp, uint256 dose);
    ...
    function addDoseRecord(uint256 patientId, uint256 dose) external payable nonReentrant whenNotPaused {
        ...
        records[patientId].push(Record({dose: dose}));

        // Emit an event for the dose modification.
        emit RecordModified(patientId, block.timestamp, dose);
    }
